...
class DataServerThread:
    ...
    def resolve(self, msg: bytes, isMeta: bool):
        ...
        # 解析数据包
            ...
            if self.meta['flag'] % 2 == 0:
                ...
                # ------ 新增：处理长度不一致 ------
                lengths = [len(ch) for ch in tempBuf]
                if len(set(lengths)) > 1:
                    min_len = min(lengths)
                    print(f"警告：通道数据长度不一致，截断至 {min_len} 点")
                    tempBuf = [ch[:min_len] for ch in tempBuf]
                # ---------------------------------
    ...
    def GetBufferUpdate(self):
        """
        获取buffer中的新数据（增量数据）
        :return:
        """
        return self.buffer.getUpdate()
